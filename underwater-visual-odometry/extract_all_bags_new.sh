...
# Extract all 5 ROS bags using new script structure
echo "Starting extraction of all 5 ROS bags using new structure..."
...
for i in "${!bags[@]}"; do
    ...
    docker exec underwater_vo bash -c "source /opt/ros/noetic/setup.bash && cd /app && python3 scripts/data_processing/extract_rosbag_data.py --bag data/raw/$bag --output data/processed/bag_$i"

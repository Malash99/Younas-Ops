#!/usr/bin/env python3
"""
Baseline Trajectory Prediction: Bag 0, Camera 0
Predict the complete trajectory of Bag 0 using Camera 0 data
(the same camera the model was trained on)
"""
...
def get_bag0_data():
    """Get all data from Bag 0 for Camera 0"""
...
def predict_full_trajectory_bag0_cam0(model, device, bag0_data, window_size=3):
    """Predict the complete Bag 0 trajectory using Camera 0 (trained camera)"""
    
    print(f"\nPREDICTING FULL BAG 0 TRAJECTORY - CAMERA 0 (BASELINE)")
    print("=" * 60)
    ...
    with torch.no_grad():
        pbar = tqdm(range(len(bag0_data) - window_size + 1), desc="Predicting Bag 0 - Cam0 (Baseline)")
        ...
        for i in pbar:
            try:
                ...
                # Create dummy images (Camera 0 - trained camera)
                # In real implementation, you would load actual camera 0 images here
                images_list = []
                for _, row in window_data.iterrows():
                    # Generate dummy image that simulates camera 0 input (trained camera)
                    # You could replace this with actual image loading from row['cam0_path']
                ...
                camera_ids_batch = torch.tensor([[0]], device=device)  # Camera 0 (trained camera)
...
def visualize_bag0_cam0_prediction(results, save_path='bag0_cam0_baseline_prediction.png'):
    """Create comprehensive visualization of Bag 0 Camera 0 baseline prediction"""
    ...
    ax1.plot(pred_traj[:, 0], pred_traj[:, 1], 'green', linewidth=3, alpha=0.8,
             label='Predicted (Cam0 - TRAINED)', linestyle='--')
    ...
    ax1.scatter(pred_traj[-1, 0], pred_traj[-1, 1], color='green', s=150, marker='s',
    ...
    ax1.set_title('Bag 0 - Camera 0 BASELINE Trajectory Prediction', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.plot(pred_traj[:, 0], pred_traj[:, 1], pred_traj[:, 2], 'green', alpha=0.8, linewidth=3)
    ...
    ax3.plot(time_steps, position_errors, 'darkgreen', linewidth=2)
    ax3.fill_between(time_steps, position_errors, alpha=0.3, color='green')
    ...
    ax4.plot(time_steps, pred_traj[:, 0], 'g--', linewidth=2, label='Pred X')
    ax4.plot(time_steps, gt_traj[:, 0], 'b-', linewidth=2, label='GT X')
    ax4.plot(time_steps, pred_traj[:, 1], 'g:', linewidth=2, label='Pred Y')
    ...
    metrics_text = f"""
BAG 0 - CAMERA 0 BASELINE RESULTS
{'='*40}
...
"""
    ...
    plt.suptitle('UW-TransVO: Bag 0 BASELINE Trajectory Prediction with Camera 0', 
...
def main():
    ...
    print("BAG 0 - CAMERA 0 BASELINE TRAJECTORY PREDICTION")
    print("=" * 60)
    print("Testing BASELINE performance:")
    print("• Model trained on: Camera 0")  
    print("• Testing on: Camera 0 (SAME CAMERA)")
    print("• Trajectory: Bag 0 (complete loop, 1074 frames)")
    print("• Expected: BEST possible performance")
    print("=" * 60)
    ...
    results = predict_full_trajectory_bag0_cam0(model, device, bag0_data)
    ...
    # Visualize results
    metrics = visualize_bag0_cam0_prediction(results)
    
    # Print final summary
    print(f"\n{'='*60}")
    print("FINAL RESULTS - BAG 0 CAMERA 0 BASELINE:")
    print(f"{'='*60}")
    ...
    print(f"\nBaseline performance: {'EXCELLENT' if metrics['relative_error'] < 20 else 'GOOD' if metrics['relative_error'] < 50 else 'NEEDS_IMPROVEMENT'}")
    
    # Compare with Camera 2 results (if available)
    print("\n" + "="*60)
    print("COMPARISON WITH CAMERA 2 GENERALIZATION:")
    print("="*60)
    print("Camera 0 (baseline): Trained camera performance")
    print("Camera 2 (previous): Cross-camera generalization")
    print("Expected: Camera 0 should perform better than Camera 2")

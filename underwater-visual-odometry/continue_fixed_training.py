"""
Continue Fixed Anti-Collapse Training

Continue training the fixed model that already shows promising results
to achieve even better curved trajectory prediction.
"""
...
# Import the dataset and loss from the original script
...
def continue_training():
    """Continue training the fixed model"""
    ...
    # Training parameters
    batch_size = 4
    learning_rate = 8e-6  # Slightly lower for continued training
    num_additional_epochs = 15  # Continue for more epochs
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("=" * 60)
    print("CONTINUING ANTI-COLLAPSE TRAINING")
    print("=" * 60)
    print(f"Device: {device}")
    print(f"Learning rate: {learning_rate} (reduced for continued training)")
    ...
    if not os.path.exists(model_path):
        print(f"ERROR: Cannot continue - model not found at {model_path}")
        return
    ...
    dataset = AntiCollapseDataset(csv_file, sequence_length=10, img_size=config['img_size'], stride=4)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, num_additional_epochs, eta_min=learning_rate*0.1)
        ...
        if train_losses and val_losses:
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'config': config
                }, 'final_fixed_model.pth')
                print(f"*** NEW BEST MODEL SAVED! Val Loss: {avg_val_loss:.6f} ***")
        
        # Test model diversity every few epochs
        if (epoch + 1) % 5 == 0:
            print(f"\\nTesting model diversity at epoch {current_epoch}...")
            test_model_diversity(model, config, device)
    
    # Save final results
    with open('continued_training_history.json', 'w') as f:
    ...
    print("="*60)
    print("Files created:")
    print("- final_fixed_model.pth (best model)")
    print("- continued_training_history.json")
    
    return training_history


def test_model_diversity(model, config, device):
    """Quick test of model diversity during training"""
    model.eval()
    
    # Create test input
    images = torch.randn(1, 10, 1, 3, config['img_size'], config['img_size']).to(device)
    ...
    # Calculate diversity
    frame_std = np.std(pred_deltas[:, :3], axis=0).mean()
    max_diff = np.abs(np.diff(pred_deltas, axis=0)).max()
    
    print(f"    Frame diversity (mean std): {frame_std:.6f}")
    print(f"    Max frame difference: {max_diff:.6f}")
    
    if frame_std > 0.005:
        print(f"    ✓ Good diversity maintained!")
    elif frame_std > 0.001:
        print(f"    ~ Moderate diversity")
    else:
        print(f"    ⚠ Low diversity - may need adjustment")


if __name__ == "__main__":
    print("Continuing Fixed Anti-Collapse Training")
    print("This will improve the already working curved trajectory model")
    
    history = continue_training()
    
    if history:
        print("\\nCONTINUED TRAINING COMPLETED!")
        print("The model should now have even better curved trajectory prediction!")
        print("Test final results with: python test_fixed_model.py")
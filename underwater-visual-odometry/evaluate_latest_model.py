#!/usr/bin/env python3
"""
Evaluate the latest model (3_TSFormer_seq3_advanced_loss) and generate trajectory plots
"""
...
def plot_trajectories(pred_trajectory, true_world_coords, dataset_name, output_dir):
    ...
    plt.suptitle(f'Advanced Loss Model Results - {dataset_name} Dataset\\nBlue=TRUE World Coordinates, Red=Model Predictions', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_dir / f'advanced_loss_results_{dataset_name.lower()}.png', 
               dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Saved: advanced_loss_results_{dataset_name.lower()}.png")

def main():
    """Main evaluation function"""
    print("=== EVALUATING ADVANCED LOSS MODEL ===")
    
    # Create output directory
    output_dir = Path('evaluation_results_3_TSFormer_seq3_advanced_loss')
    output_dir.mkdir(exist_ok=True)
    
    # Load model
    print("Loading model...")
    model, device, config, checkpoint = load_model()
    ...
    for dataset_name, data_loader in [('Test', data_loaders['test_loader'])]:
        ...
        # Get predictions
        predictions, metadata = get_predictions(model, device, data_loader)
        
        # Get true world coordinates
        true_world_coords = get_true_world_coordinates(config['csv_path'], metadata)
        
        # Accumulate predictions to create trajectory
        ...
        trans_errors = np.linalg.norm(predictions[:, :3], axis=1)  # Distance from zero
        rot_errors = np.linalg.norm(predictions[:, 3:], axis=1)
        
        print(f"\\n{dataset_name} Dataset Metrics:")
        print(f"  Translation predictions mean magnitude: {np.mean(trans_errors):.4f} m")
        print(f"  Rotation predictions mean magnitude: {np.mean(rot_errors):.4f} rad ({np.degrees(np.mean(rot_errors)):.1f}°)")

...
def matrix_to_pose(T):
    ...
    return {'position': position, 'quaternion': q.elements}


def orthogonalize(R):
    """Force a matrix to be orthogonal using SVD."""
    U, _, Vt = np.linalg.svd(R)
    return U @ Vt


def relative_to_matrix(translation, rotation):
    """Convert relative pose (translation + rotation) to 4x4 matrix."""
    q = Quaternion(axis=rotation[:3], angle=np.linalg.norm(rotation)) if np.linalg.norm(rotation) > 0 else Quaternion()
    R = q.rotation_matrix
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = translation
...
def transform_pose_to_global(relative_pose, current_pose):
    """
    Transform a relative pose (dx, dy, dz, d_rx, d_ry, d_rz) into the global frame,
    starting from current global pose.
    """
    # Extract translation and rotation vector
    trans = relative_pose[:3]
    rot_vec = relative_pose[3:]

    # Convert current pose to matrix
    current_q = Quaternion(current_pose['quaternion'])
    R_global = current_q.rotation_matrix
    t_global = current_pose['position']

    # Convert relative rotation vector to matrix
    angle = np.linalg.norm(rot_vec)
    if angle > 0:
        axis = rot_vec / angle
        rel_q = Quaternion(axis=axis, angle=angle)
    else:
        rel_q = Quaternion()
    rel_R = rel_q.rotation_matrix

    # Compose new rotation
    new_R_global = orthogonalize(R_global @ rel_R)

    # Compose new translation
    new_t_global = t_global + R_global @ trans

    return {
        'position': new_t_global,
        'quaternion': Quaternion(matrix=new_R_global).elements
    }


def integrate_trajectory(initial_pose, relative_poses):
    """
    Integrate a sequence of relative poses into a trajectory in global coordinates.
    Each relative pose is assumed to be [dx, dy, dz, d_rx, d_ry, d_rz]
    where rotation is a 3D rotation vector (axis * angle).
    """
    trajectory = [initial_pose['position']]
    current_pose = initial_pose

    for rel_pose in relative_poses:
        current_pose = transform_pose_to_global(rel_pose, current_pose)
        trajectory.append(current_pose['position'])

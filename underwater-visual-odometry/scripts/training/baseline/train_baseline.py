...
def train_model(args):
    ...
    # Initialize dataset with FIXED coordinate transformations
...
def evaluate_model(model, val_dataset, dataset, val_indices):
    ...
    if len(val_indices) > 100:
        ...
        # Integrate predicted trajectory using CORRECTED method
        pred_trajectory = integrate_trajectory_correct(initial_pose, predictions[:99])
        
        # Integrate ground truth trajectory using CORRECTED method
        gt_trajectory = integrate_trajectory_correct(initial_pose, ground_truth[:99])
        ...
        print(f"RPE (trans): {traj_errors['rpe_trans_mean']:.4f} ± {traj_errors['rpe_trans_std']:.4f} m")
        print(f"RPE (rot): {traj_errors['rpe_rot_mean']:.4f} ± {traj_errors['rpe_rot_std']:.4f} rad")
